...
function timeSpanToString(startDate, endDate) {
  const value = new Date(endDate - startDate);
...
function angleBetweenClockHands(date) {
  const hour = ((date.getUTCHours() % 12) * 30 + date.getUTCMinutes() * 0.5);
  const minute = (date.getMinutes() * 6);
  const delta = Math.abs(hour - minute);
  return (Math.PI * (Math.min(360 - delta, delta))) / 180;
}
